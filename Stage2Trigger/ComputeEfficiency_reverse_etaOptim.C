...
int main(int argc, char** argv)
{
    ...
    if (argc < 4)
    {
        cout << "Usage: [skipHPSIso] [effWP] [AskL1Iso]" << endl;
    ...
    else
    {
        ...
        effWPName = atoi(argv[2]);
        
        if (atof(argv[3]) != 0)
        {
            cout << "AskL1Iso: true" << endl;
            AskL1Iso = true;
        }
        else
        {
            cout << "AskL1Iso: false" << endl;
            AskL1Iso = false;
        }
    }
    //////////////////////
    ...
    TString inFile = Form("/data_CMS/cms/cadamuro/L1_trigger_data/MergeTrees_GluGlu/MergeTree_gg_LUT%iStage2_AlwaysCBM_0.root", effWPName);
    TString outFile = Form("eff_L1Tau_Stage2_gg_LUTProcessed%i_AlwaysCBM", effWPName);
...
*/
    ...
    for (int iEv = 0; iEv < nEvents; iEv++)
    {
        ...
        for (int iGen = 0; iGen < nGen; iGen++)
        {
            ...
            if (L1Index > -1) _GoodL1TauPt.push_back((*TauCalibPt)[L1Index]);
